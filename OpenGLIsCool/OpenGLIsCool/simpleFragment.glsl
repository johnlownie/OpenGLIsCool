...
// Mask or discard texture
uniform sampler2D textureDiscard;
uniform bool bUseDiscardTexture;

// Checkpoint 11
uniform sampler2D textureTranspose;
uniform bool bUseTransposeTexture;
...
void main()
{
    ...
    {
        vec4 texRGBAPixel_TD = texture(textureDiscard, fUVx2.st);
        if (texRGBAPixel_TD.r > 0.5f)
        {
            discard;
        }
    }

    if (bUseTransposeTexture)
    {
        
    }

    // Sample a pixel from the texture sampler
    vec4 texRGBAPixel_0A = texture(texture0A, fUVx2.st);
    vec4 texRGBAPixel_0B = texture(texture0B, fUVx2.st);
    vec4 texRGBAPixel_0C = texture(texture0D, fUVx2.st);
    vec4 texRGBAPixel_0D = texture(texture0C, fUVx2.st);

    vec4 finalTexColour = (texRGBAPixel_0A * textureRatios.x) + (texRGBAPixel_0B * textureRatios.y) + (texRGBAPixel_0C * textureRatios.z) + (texRGBAPixel_0D * textureRatios.w);
    
    outputColour = calculateLightContrib(finalTexColour.rgb, vec3(fNormal.xyz), vec3(fVertWorldPos), specularColour.rgba);

    outputColour.a = diffuseColourRGBA.a;
}
